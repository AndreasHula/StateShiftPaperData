...
for(i in 1:10){
  I_Investments[i] =  t.test(FracInvestorWithBPD[,i], FracInvestorWithHC[,i], alternative = "two.sided")$p.value;
}
T_Repayments = vector("double",10)
for(i in 1:10){
  T_Repayments[i] =  t.test(FracBPDTrustee[,i], FracHCTrustee[,i], alternative = "two.sided")$p.value;
}
...
#example breaking interaction - Interaction pair 29 in the data
